...
class NesterovARunFuncTestsProcesses : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 ...
 protected:
  void SetUp() override {
    ...
    {
      std::string abs_path = ppc::util::GetAbsoluteTaskPath(PPC_ID_nalitov_d_matrix_min_by_columns, "pic.jpg");
...
const auto kTestTasksList =
    std::tuple_cat(ppc::util::AddFuncTask<NalitovDMinMatrixMPI, InType>(kTestParam, PPC_SETTINGS_nalitov_d_matrix_min_by_columns),
                   ppc::util::AddFuncTask<NalitovDMinMatrixSEQ, InType>(kTestParam, PPC_SETTINGS_nalitov_d_matrix_min_by_columns));
